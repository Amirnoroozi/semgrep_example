# cf. https://github.com/python-security/pyt//blob/093a077bcf12d1f58ddeb2d73ddc096623985fb0/
...
def switch(target:str, config: Dict[str, str]):
    if does_base_exist(target,config):
        ...
        # ruleid: make-response-with-unknown-content
        resp = (make_response(({'Switched': "The current knowledge base is now : '" + 
        target + "'"}), 200))
    else:
        resp = (make_response(({'Error': "The knowledge base '" + target + "' does not 
        exist"}), 404))
