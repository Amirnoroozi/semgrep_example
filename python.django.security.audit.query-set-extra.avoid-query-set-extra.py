...
findings = Finding.objects.filter(verified=True,
        ...
        select={
            'ra_count': 'SELECT COUNT(*) FROM dojo_risk_acceptance INNER JOIN '
                        ...
                        '( dojo_risk_acceptance.id = dojo_risk_acceptance_accepted_findings.
                        risk_acceptance_id ) '
                        'WHERE dojo_risk_acceptance_accepted_findings.finding_id = 
                        dojo_finding.id',
        },
    )
example = 1
active_findings = Finding.objects.filter(verified=True, active=True,
            'ra_count': f'SELECT COUNT(*) FROM dojo_risk_acceptance INNER JOIN '
                        ...
                        f'( dojo_risk_acceptance.id = 
                        dojo_risk_acceptance_accepted_findings.risk_acceptance_id ) '
                        f'WHERE dojo_risk_acceptance_accepted_findings.finding_id = 
                        {example}',
