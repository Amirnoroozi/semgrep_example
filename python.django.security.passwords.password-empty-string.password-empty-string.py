...
def test_email_auth_backend_empty_password(user_profile: UserProfile) -> None:
    ...
    self.assertIsNotNone(EmailAuthBackend().authenticate(username=self.example_email
    ('hamlet'), password=password))
    # Now do the same test with the empty string as the password.
    # ruleid: password-empty-string
    password = ""
    self.assertIsNone(EmailAuthBackend().authenticate(username=self.example_email
    password = ''
